...
# 保存评论
def insert_comments(music_id, user_id, user_name, comment):
    with connection.cursor() as cursor:
        sql = "INSERT INTO `comments` (`MUSIC_ID`, `USER_ID`, `USER_NAME`,`COMMENT`) VALUES (%s, %s, %s,%s)"
        cursor.execute(sql, (music_id, user_id, user_name, comment))
    connection.commit()
...
def insert_music(music_id, music_name, album_id):
    with connection.cursor() as cursor:
        sql = "INSERT INTO `music` (`MUSIC_ID`, `MUSIC_NAME`, `ALBUM_ID`) VALUES (%s, %s, %s)"
...
def get_all_music():
    with connection.cursor() as cursor:
        sql = "SELECT `MUSIC_ID` FROM `music` ORDER BY MUSIC_ID"
        cursor.execute(sql, ())
        return cursor.fetchall()
